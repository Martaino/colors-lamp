...
the version-control assignment; it is an educational demo.
...
1. Install PHP with MySQLi/mysqlnd, MySQL, and (for LAMP deployment) Apache
   with PHP support. Use a supported PHP 8.x installation. A PHP development
...
The existing initial README commit is preserved. Subsequent commits organize
this supplied project into structure/API configuration, backend/schema, and
cleanup/documentation stages. These are staged imports of existing lab work,
not claims about when the original application was developed.

OpenAI Codex assisted with organizing the provided lab files, replacing embedded
connection settings with environment variables, adapting the API URL, adding
an empty schema, minor cleanup, documentation, licensing, and Git operations.
The application originated from the supplied COLORS lab materials. Review this
AI disclosure against your course's policy before submitting; no specific
course AI policy was supplied with this task.
